...
public class NodeManager : Singleton<NodeManager>
{
    ...
    [SerializeField]
    private float detectRange = 120f;
    ...
    private void Update()
    {
        ...
        foreach (var node in nodes) //노드검사
        {
            ...
            {                                       //주변 노드 콜리더 검사
                var dir = nearCollider.transform.position - currentNodePos;
                float leftRightCheck = Vector3.Dot(Vector3.right, dir.normalized);
                float upDownCheck = Vector3.Dot(Vector3.forward, dir.normalized);
                var nearNode = nearCollider.GetComponent<Node>();
